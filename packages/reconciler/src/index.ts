import ReconcilerImpl, { HostConfig } from 'react-reconciler'
...
import { EventHandlers, RootState } from './types'
...
export type Root = { fiber: ReconcilerImpl.FiberRoot; store: UseStore<RootState> }
...
export let extend = (objects: object): void => void (catalogue = { ...catalogue, ...objects })
...
function createInstance(
  ...
  hostContext?: any,
  internalInstanceHandle?: ReconcilerImpl.Fiber,
) {
  ...
  if (!isStore(root) && internalInstanceHandle) {
    const fn = (node: ReconcilerImpl.Fiber): UseStore<RootState> => {
...
function switchInstance(instance: Instance, type: string, newProps: InstanceProps, fiber: ReconcilerImpl.Fiber) {
  ...
  ;[fiber, fiber.alternate].forEach((fiber) => {
    if (fiber !== null) {
      ...
      if (fiber.ref) {
        ...
        else (fiber.ref as ReconcilerImpl.RefObject).current = newInstance
      }
    }
  })
}
...
>(
  ...
  const extendedConfig = Object.fromEntries(
    Object.entries(hostConfig).map(([key, value]) => {
      if (value instanceof Function && (config as any)[key]) {
        return [key, (config as any)[key](value)]
      } else {
        return [key, value]
      }
    }),
  ...
  return ReconcilerImpl(extendedConfig as any)
}
